...
qplot(x = bigram, y = count, data = termFreq, 
...
qplot(x = trigram, y = count, data = termFreq, main = "Trigram Frequencies",
...
ugram.freq$pdist <- ugram.freq$count / sum(ugram.freq$count)

bgram.freq$n1 <- factor(rapply(strsplit(as.character(bgram.freq$bigram), " "),
                               function(x) x[1]))
bgram.freq$p.n1 <- lapply(bgram.freq$n1, function(x) sum(bgram.freq$n1 == x) / nrow(bgram.freq))
bgram.freq$n2 <- factor(rapply(strsplit(as.character(bgram.freq$bigram), " "),
                               function(x) x[2]))
bgram.freq$n2.count <- lapply(bgram.freq$n2, function(x) sum(bgram.freq$n2 == x))

# this isnt working right, need to pass in 2 factors
# desired result:
# for each word (n1), what % of the time is n2 == n2 for a given n1?
bgram.freq$p.n2.n1 <- lapply(bgram.freq$n2, 
                             function(x) sum(bgram.freq$n2[bgram.freq$n1 == bgram.freq$n1] == x) / nrow(bgram.freq[bgram.freq$n1 == bgram.freq$n1, ]))




tgram.freq$n1 <- rapply(strsplit(as.character(tgram.freq$word), " "),
                        function(x) x[1])
tgram.freq$n2 <- rapply(strsplit(as.character(tgram.freq$word), " "),
                        function(x) x[2])
tgram.freq$n3 <- rapply(strsplit(as.character(tgram.freq$word), " "),
                        function(x) x[3])
```
