...
#       x <- tm_map(x, removeWords, stopwords("english")) # consider removing this
#       x <- tm_map(x, content_transformer(function(z) gsub("[^[:alnum:]///' ]", "", z))) # try this encoding again [^0-9A-Za-z///' ]
      x <- tm_map(x, content_transformer(function(z) gsub("[^0-9A-Za-z///' ]", "", z))) # this seems to have the desired effect, not the above
      x <- tm_map(x, stemDocument)
      x <- tm_map(x, stripWhitespace)
      x
} # still need to find a way to filter out the a-hat, maybe use encoding?
...
qplot(x = word, y = count, data = termFreq, main = "Term Frequencies",
      geom = "bar", xlab = "Terms", stat = "identity") + coord_flip()

termFreq <- with(bgram.freq, bgram.freq[count >= quantile(count, 0.95), ])
qplot(x = word, y = count, data = termFreq, main = "Bigram Frequencies",
      geom = "bar", xlab = "Bigrams", stat = "identity") + coord_flip()
...
qplot(x = word, y = count, data = termFreq, main = "Trigram Frequencies",
      geom = "bar", xlab = "Trigrams", stat = "identity") + coord_flip()

# try a graph as percentages
termFreq <- data.frame(freq, percent = freq$count / sum(freq$count) * 100)
termFreq <- termFreq[termFreq$percent > 0.25, ]
qplot(x = word, y = percent, data = termFreq, main = "Term Frequency Percent",
      geom = "bar", xlab = "Terms", stat = "identity") + coord_flip()




# findAssocs(tdm, "here", 0.1)
