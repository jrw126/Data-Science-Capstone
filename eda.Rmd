...
bgram.freq$n1 <- factor(rapply(strsplit(as.character(bgram.freq$word), " "),
                               function(x) x[1]))
bgram.freq$n2 <- factor(rapply(strsplit(as.character(bgram.freq$word), " "),
                               function(x) x[2]))

bgram.pdist <- aggregate(count ~ n2 + n1, bgram.freq, sum)
bgram.pdist <- bgram.pdist[, c(2, 1, 3)]
